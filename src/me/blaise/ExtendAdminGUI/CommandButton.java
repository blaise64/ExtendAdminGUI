...
public class CommandButton extends GenericButton
{
	...
	public CommandButton(Element node, int rowheight, int colwidth, GridContainer parent, ExtendAdminGUI app_root)
	{
		...
		if(this.app_root.plugin.config.getBoolean("debug_mode"))
			this.app_root.log((new StringBuilder("[ExtendAdminGUI - debug] new command button :")).append(" command "+command).append(" col "+col).append(" row "+row).append("size is: h"+this.getHeight()+" w "+this.getWidth()).toString());
	...
	public void onButtonClick(ButtonClickEvent event)
	{
		...
		for(int i = 0; i < commands.length; i++)
		{
			...
			//debug
			if(this.app_root.plugin.config.getBoolean("debug_mode"))
				this.app_root.log((new StringBuilder("[ExtendAdminGUI - debug] ")).append(user.getName()).append(" executed the command/chat '").append(s).append("'").toString());
			i++;
		}
		//close the GUI
