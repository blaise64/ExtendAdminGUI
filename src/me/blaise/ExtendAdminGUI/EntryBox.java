...
public class EntryBox extends GenericTextField
{
	...
	public void readData(DataInputStream input) throws IOException
	{
		...
		System.out.println("entry : reading data- focus:"+isFocused());
	}
	
	public void writeData(DataOutputStream output) throws IOException
	{
		super.writeData(output);
		System.out.println("entry : writing data - focus:"+isFocused());
	}
    ...
    public EntryBox(Element node, int rowheight, int colwidth, GridContainer parent, ExtendAdminGUI app_root)
    {
        ...
        if(this.app_root.plugin.config.getBoolean("debug_mode"))
    		this.app_root.log((new StringBuilder("[ExtendAdminGUI - debug] new entrybox :")).append(" param "+this.var).append(" col "+col).append(" row "+row).toString());
    }
